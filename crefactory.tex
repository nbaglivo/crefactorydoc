...
En segundo lugar, la herramienta no expresaba toda su funcionalidad a trav\'es de su la interfaz, como es el caso de de la espeficicaci\'on de configuraciones (las cuales ser\'an abordadas a la secci\'on~\ref{sec:problema_dos}) con lo cual se desaprovechaba  parte del poder de CRefactory.
...
Particularmente se desea mejorar el mecanismo para la carga de archivos en orden lograr prevenir errores e incrementar su usabilidad.

Implementar una interfaz que permita al usuario hacer uso de todo el potencial de CRefactory, en particular al crear configuraciones para sus proyectos.
...
Luego de la reuni\'on con la Profesora organizabamos una planning, en la cual uno de nosotros cumpl\'ia el rol de scrum master. Durante la reuni\'on decidiamos cuales ser\'ian nuestros objetivos a alcanzar en el sprint y creabamos las tareas necesarias para cumplirlo, de ah\' en adelante cada miembro era libre de escoger la tarea disponible que la pareciera. Para una mejor organizaci\'on, y sobre todo para evitar solapamiento en la realizaci\'on de las tareas, utilizamos la herramienta Trello que nos permiti\'o mantenernos sincronizados y al tanto del avance y trabajo de cada miembro, otorgandonos la posibilidad de mantener una organizaci\'on a\'un trabajando a distancia y en horarios diferentes.
